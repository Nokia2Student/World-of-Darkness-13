		...
		if(stat != DEAD)
			if(!is_base_breed() || auspice?.breed_form == FORM_CRINOS)
				...
				if(lastattacked_time + 30 SECONDS < world.time)
					var/stamina_roll = secret_vampireroll(get_a_stamina(src), 8, src)
					if(stamina_roll >= 3)
						adjustFireLoss(-25, TRUE)
				else
					adjustFireLoss(-25, TRUE)
	if(auspice.rage >= 1 && stat < CONSCIOUS && lastattacked_time + 1 SCENES < world.time)
		var/rage_roll = secret_vampireroll(auspice.start_rage, 8, src)
		if(rage_roll >= 1)
			adjust_rage(-1, src)
